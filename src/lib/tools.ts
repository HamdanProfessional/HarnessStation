...
export async function executeTool(
  ...
  const presetActive =
    (gs.toolSandbox && gs.toolSandbox !== "full-access") ||
    (gs.toolApproval && gs.toolApproval !== "full-auto");
  if (presetActive || (gs.blockTools?.length ?? 0) + (gs.confirmTools?.length ?? 0) + (gs.guardrails?.length ?? 0) > 0) {
